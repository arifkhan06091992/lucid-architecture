...
$(document).ready(function() {
    dataTable = $('#custom-databale').DataTable( {
        ...
        "serverSide": true,
        "ajax":{
            url : BASE_URL+'/admin/email-notifications/getDatatableData', // json datasource
...
function statusChange(id,elmnt){
    ...
    bootbox.confirm({
        message: 'Are you sure you want to change email notification status ?',
        buttons: {
            'cancel': {
        ...
        callback: function(result) {
            if (result) {
                $('#ajaxLoader').show();
                $.ajax({
                    url: BASE_URL+'/admin/email-notifications/ajaxChangeStatus',
                    type: 'POST',
                    ...
                    error: function () {
                        console.log('There is some error in email notification status change. Please try again.');
                    }
                });
                return false;
            }
