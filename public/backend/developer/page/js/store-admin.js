$(document).ready(function() {
    ...
    dataTable = $('#custom-datatable').DataTable( {
        ...
        "serverSide": true,
        "ajax":{
            url : BASE_URL+'/admin/store-admin/getDatatableData', // json datasource
        ...
        "columnDefs": [ { orderable: false, targets: [0,1,2,8] }],
        "order": [[ 0, 'desc' ]]
    } );
    $('.dataTables_filter input').attr("placeholder", "Store Admin Name, Email").css('width','200px');
    ...
    $('#autocomplete_list').autocomplete({
        ...
        onSelect: function (suggestion) {
            ...
            dataTable.columns( 1 ).search( suggestion.data ).draw();
        }
    });
    ...
    bootbox.confirm({
        ...
        callback: function(result) {
            if (result) {
                $('#ajaxLoader').show();
                $.ajax({
                    url: BASE_URL+'/admin/store-admin/'+id,
                    type: 'DELETE',
                    data: {  _token : _token},
    ...
    bootbox.confirm({
        ...
        callback: function(result) {
            if (result) {
                $('#ajaxLoader').show();
                $.ajax({
                    url: BASE_URL+'/admin/store-admin/ajaxChangeStatus',
                    type: 'POST',
    ...
    bootbox.confirm({
        ...
        callback: function(result) {
            if (result) {
                $('#ajaxLoader').show();
                $.ajax({
                    url: BASE_URL+'/admin/store-admin/changePassword/'+id,
                    type: 'POST',
                    data: {  _token : _token},
                    success: function (data) {
                        location.reload();
