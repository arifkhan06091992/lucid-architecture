...
$(document).ready(function() {
    dataTable = $('#custom-databale').DataTable( {
        ...
        "serverSide": true,
        "ajax":{
            url : BASE_URL+'/admin/cms-manager/getDatatableData', // json datasource
    ...
    $('.dataTables_filter input').attr("placeholder", "Page Title").css('width','200px');
...
function statusChange(id,elmnt){
    ...
    bootbox.confirm({
        message: 'Are you sure you want to change cms page status ?',
        buttons: {
            'cancel': {
        ...
        callback: function(result) {
            if (result) {
                $('#ajaxLoader').show();
                $.ajax({
                    url: BASE_URL+'/admin/cms-manager/ajaxChangeStatus',
                    type: 'POST',
                    ...
                    error: function () {
                        console.log('There is some error in cms page status change. Please try again.');
                    }
                });
                return false;
            }
