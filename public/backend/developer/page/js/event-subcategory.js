...
$(document).ready(function() {
    dataTable = $('#custom-datatable').DataTable( {
        ...
        "serverSide": true,
        "ajax":{
            url : BASE_URL+'/admin/event-subcategory/getDatatableData', // json datasource
    ...
    $('.dataTables_filter input').attr("placeholder", "Subcategory Name").css('width','200px');
...
function deleteRecord(id){
    ...
    bootbox.confirm({
        message: 'Are you sure you want to delete this event subcategory ?',
        buttons: {
            'cancel': {
        ...
        callback: function(result) {
            if (result) {
                $('#ajaxLoader').show();
                $.ajax({
                    url: BASE_URL+'/admin/event-subcategory/'+id,
                    type: 'DELETE',
                    data: {  _token : _token},
...
function statusChange(id,elmnt){
    ...
    bootbox.confirm({
        message: 'Are you sure you want to change event subcategory status ?',
        buttons: {
            'cancel': {
                label: '<span><span>Cancel</span></span>',
                className: 'btn btn-primary'
        ...
        callback: function(result) {
            if (result) {
                $('#ajaxLoader').show();
                $.ajax({
                    url: BASE_URL+'/admin/event-subcategory/ajaxChangeStatus',
                    type: 'POST',
