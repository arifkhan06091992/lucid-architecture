$(document).ready(function() {
    dataTable = $('#custom-databale').DataTable( {
        ...
        "serverSide": true,
        "ajax":{
            url : BASE_URL+'/admin/group/getDatatableData', // json datasource
    ...
    $('.dataTables_filter input').attr("placeholder", "Group Name").css('width','200px');
...
function statusChange(id,elmnt){
    ...
    bootbox.confirm({
        message: 'Are you sure you want to change Group ?',
        buttons: {
            'cancel': {
                label: '<span><span>Cancel</span></span>',
                className: 'btn btn-primary'
        ...
        callback: function(result) {
            if (result) {
                $('#ajaxLoader').show();
                $.ajax({
                    url: BASE_URL+'/admin/group/ajaxChangeStatus',
                    type: 'POST',
