$(document).ready(function() {
    dataTable = $('#custom-databale').DataTable( {
        ...
        "serverSide": true,
        "ajax":{
            url : BASE_URL+'/admin/ticket-type/getDatatableData', // json datasource
    ...
    $('.dataTables_filter input').attr("placeholder", "Ticket Type Name").css('width','200px');
    ...
    bootbox.confirm({
        ...
        callback: function(result) {
            if (result) {
                $('#ajaxLoader').show();
                $.ajax({
                    url: BASE_URL+'/admin/ticket-type/'+id,
                    type: 'DELETE',
                    data: {  _token : _token},
...
function statusChange(id,elmnt){
    ...
    bootbox.confirm({
        message: 'Are you sure you want to change Ticket type ?',
        buttons: {
            'cancel': {
                label: '<span><span>Cancel</span></span>',
                className: 'btn btn-primary'
        ...
        callback: function(result) {
            if (result) {
                $('#ajaxLoader').show();
                $.ajax({
                    url: BASE_URL+'/admin/ticket-type/ajaxChangeStatus',
                    type: 'POST',
